...
class DLPMainController(QObject):
    ...
    super_job_settings_updated = Signal()
    super_job_loaded = Signal(bool)
    ...
    def __init__(self, printer_setup='BOTTOM-UP', projector_setup='VisitechDLP9000', motor_setup='ClearpathSDSK'):
        ...
        # Printing parameters
        self.super_job_filename = ""
        self.super_job_settings = []
        self.is_super_job_loaded = False
        self.layers_amplitudes = []
        self.layers_exposures = []
        self.layers_thicknesses = []
        self.layers_filenames = []
    ...
    @Slot(bool)
    def set_super_job_loaded(self, status):
        self.is_super_job_loaded = status
        self.super_job_loaded.emit(status)

    @Slot(str)
    def load_super_job_file(self, filename):
        job_file = QFile(filename)
        job_file_dir = QDir(QFileInfo(filename).absolutePath())
        self.layers_filenames = []
        self.super_job_settings = []
        self.number_of_layers = 0
        self.set_super_job_loaded(False)
        self.etc_updated_signal.emit(0.0)
        if job_file.open(QIODevice.ReadOnly | QIODevice.Text):
            job_data = QJsonDocument.fromJson(job_file.readAll()).object()
            if "groups" in job_data:
                groups = job_data["groups"]
                self.super_job_etc_ms = 0
                for g in groups:
                    layers = [job_file_dir.absoluteFilePath(l) if QFileInfo(l).isRelative() else l for l in g["layers"]]
                    self.layers_filenames.extend(layers)
                    self.super_job_settings.extend(g["settings"][0:len(layers)])
                    self.super_job_etc_ms += g["group_etc_ms"]
                self.etc_updated_signal.emit(self.super_job_etc_ms)
                self.number_of_layers = len(self.layers_filenames)
                self.set_super_job_loaded(True)
            job_file.close()

    @Slot()
    def apply_super_job_settings(self, layer):
        if self.is_super_job_loaded and layer < self.number_of_layers:
            layer_settings = self.super_job_settings[layer]
            for key in layer_settings.keys():
                if key == 'grayscale_correction':
                    self.set_grayscale_correction(layer_settings[key])
                elif key == 'grayscale_alpha':
                    self.set_grayscale_alpha(layer_settings[key])
                elif key == 'grayscale_beta':
                    self.set_grayscale_beta(layer_settings[key])
                elif key == 'grayscale_gamma':
                    self.set_grayscale_gamma(layer_settings[key])
                elif key == 'horizontal_mirror':
                    self.set_horizontal_mirroring(layer_settings[key])
                elif key == 'vertical_mirror':
                    self.set_vertical_mirroring(layer_settings[key])
                elif key == 'repositioning_delay (ms)':
                    self.__motor_controller.repositioning_delay = layer_settings[key]
                elif key == 'feed_rate (mm/min)':
                    self.__motor_controller.feed_rate = layer_settings[key]
                elif key == 'repositioning_offset (mm)':
                    self.__motor_controller.repositioning_offset = layer_settings[key]
                elif key == 'layer_etc':
                    if layer == 0:
                        self.processed_time_ms = 0
                    else:
                        self.processed_time_ms += self.super_job_settings[layer-1][key]
                    self.print_text_signal.emit(
                        "ETC: " + QDateTime.fromTime_t(max(0.0, (self.super_job_etc_ms-self.processed_time_ms) / 1000.0)).toUTC().toString('hh:mm:ss'))
            self.super_job_settings_updated.emit()
    ...
    def starting_printing_process(self):
        ...
        if self.__motor_controller.is_connected or DEBUG_MODE_ON:
            ...
            self.initialize_printing_parameters()
    ...
    @Slot()
    def initialize_printing_parameters(self):
        self.current_layer = 0
        if self.is_super_job_loaded:
            if self.number_of_layers == 0:
                return
            self.layers_amplitudes = [int(self.super_job_settings[idx]['amplitude']) for idx in range(self.number_of_layers)]
            self.layers_exposures = [int(self.super_job_settings[idx]['exposure_time (ms)']) for idx in range(self.number_of_layers)]
            self.layers_thicknesses = [self.super_job_settings[idx]['layer_thickness (um)']/1000 for idx in range(self.number_of_layers)]
        else:
            ...
            self.layers_exposures = []
            self.layers_thicknesses = []
            self.layers_filenames = self.support_file_names + self.features_file_names
            self.layers_amplitudes += [self.support_burn_amplitude] * min(self.support_burn_layers, len(self.support_file_names))
            self.layers_amplitudes += [self.support_amplitude] * max(0, len(self.support_file_names) - self.support_burn_layers)
            self.layers_amplitudes += [self.features_burn_amplitude] * min(self.features_burn_layers, len(self.features_file_names))
            self.layers_exposures += [self.support_burn_exposure] * min(self.support_burn_layers, len(self.support_file_names))
            self.layers_exposures += [self.support_exposure] * max(0, len(self.support_file_names) - self.support_burn_layers)
            self.layers_exposures += [self.features_burn_exposure] * min(self.features_burn_layers, len(self.features_file_names))
            self.layers_thicknesses += [self.support_thickness] * len(self.support_file_names)
            self.layers_thicknesses += [self.features_thickness] * min(self.features_burn_layers, len(self.features_file_names))
            if not self.incremental_amplitude:
                self.layers_amplitudes += [self.features_amplitude] * max(0, len(self.features_file_names) - self.features_burn_layers)
            else:
                self.layers_amplitudes += [self.starting_incremental_amplitude + self.incremental_step_amplitude * idx for idx in range(max(0, len(self.features_file_names) - self.features_burn_layers))]
            if not self.incremental_exposure:
                self.layers_exposures += [self.features_exposure] * max(0, len(self.features_file_names) - self.features_burn_layers)
            else:
                self.layers_exposures += [self.starting_incremental_exposure + self.incremental_step_exposure * idx for idx in range(max(0, len(self.features_file_names) - self.features_burn_layers))]

            if not self.incremental_thickness and not self.fixed_layer:
                self.layers_thicknesses += [self.features_thickness] * max(0, len(self.features_file_names) - self.features_burn_layers)
            elif self.fixed_layer:
                self.layers_thicknesses += [self.features_thickness] * min(1, len(self.features_file_names) - self.features_burn_layers)
                self.layers_thicknesses += [0.0] * max(0, len(self.features_file_names) - self.features_burn_layers - 1)
            elif self.incremental_thickness:
                self.layers_thicknesses += [self.starting_incremental_thickness + self.incremental_step_thickness * idx for idx in range(max(0, len(self.features_file_names) - self.features_burn_layers))]
        ...
        try:
            ...
            if not DEBUG_MODE_ON:
                if not self.__projector_controller.set_amplitude(0):
                    ...
                    return
            if self.is_super_job_loaded:
                self.apply_super_job_settings(self.current_layer)
            else:
            ...
            if self.current_layer < self.number_of_layers:
                self.current_amplitude = self.layers_amplitudes[self.current_layer]
                self.current_exposure = self.layers_exposures[self.current_layer]
                self.current_thickness = self.layers_thicknesses[self.current_layer]
                self.current_file_name = self.layers_filenames[self.current_layer]
                if self.current_thickness == 0.0:
                    move_building_plate = False
                self.__print_current_parameters__()
            else:
                ...
                self.reactivate_parameters_signal.emit()
                return
            if DEBUG_MODE_ON or not move_building_plate:
    ...
    def evaluate_time_estimate(self, current_layer=0):
        ...
        if not (self.incremental_amplitude or self.incremental_exposure or self.incremental_thickness or self.fixed_layer):
            features_time_ms = features_layer_left * (self.features_exposure + rep_delay + up_delay_ms + features_down_delay_ms)
        elif self.fixed_layer:
            tot_repositioning_time = (up_delay_ms + features_down_delay_ms) * (features_layers - features_layer_left == 0) + rep_delay * features_layer_left
            tot_exposure_time = 0
    ...
    def __load_default_parameters__(self):
        self.__default_parameters = {
            ...
            'support_thickness (mm)': 0.010,  # mm
            'support_exposure (ms)': 1000,  # ms
            'support_amplitude': 100,
            'support_burn_layers': 5,
            'support_burn_exposure (ms)': 3000,  # ms
            'support_burn_amplitude': 300,
            # Features Parameters
            'features_thickness (mm)': 0.010,  # mm
            'features_exposure (ms)': 1000,  # ms
            'features_amplitude': 100,
            'features_burn_layers': 0,
            'features_burn_exposure (ms)': 3000,  # ms
            'features_burn_amplitude': 300,
            # Advanced Parameters
            ...
            'repositioning_offset (mm)': 5.0,  # mm
